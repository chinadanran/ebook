...
jQuery(document).ready(function() {
   ...
   jQuery('.minimize').click(function(){
      ...
      if(!jQuery(this).hasClass('maximize')) {
         p.find('.container-body').slideUp(200);
         t.addClass('maximize');
         t.html('▼');
      } else {
         p.find('.container-body').slideDown(200);
   ...
   function update_storage_value() {
     var data = [];
     $('.maximize').each(function () {
       data.push($(this).data('id'));
     });
     localStorage.setItem('detail_storage', JSON.stringify(data));
   }
